...
from aura.harvesters.gtk import ARCGIS_BASE, GtkHarvester
...
class TestConfig:
    ...
    def test_wms_only_datasets_have_one_resource(self):
        ...
        wms_only = [
            c for c in h.datasets_config
            if all(r["format"] == "WMS" for r in c["resources"])
        ]
        assert len(wms_only) == 2
        for cfg in wms_only:
    ...
    def test_urls_point_to_arcgis(self):
        ...
        for cfg in h.datasets_config:
            for r in cfg["resources"]:
                assert r["url"].startswith(ARCGIS_BASE)
